 ...
 */
package com.ustc.wsn.mobileData.bean.fft;
...
public class FastFourierTransform {
     ...
     */
    public void applyComplex(float[] x, int xOff, boolean inverse, float[] out, int outOff) {
     ...
     * will need to hold twice as many float values as the input array.
     * <p>
     * Complex samples must be stored in an array in a
     * tightly packed format: <br>
     * <tt>[... r0, r1, r2, ...]</tt><br>
     ...
     * @param outOff  Start position into output array.
     */
    public void applyReal(float[] x, int xOff, boolean inverse, float[] out, int outOff) {
        // Bit-reverse the order of the data and copy into the output array.
    ...
    static int reverse(int val) {
        return (REVERSE_TABLE[val & 0xff] << 24) | (REVERSE_TABLE[(val >> 8) & 0xff] << 16) | (REVERSE_TABLE[(val >> 16) & 0xff] << 8) | (REVERSE_TABLE[(val >> 24) & 0xff]);
    }


    static void transform(float[] x, int off, int len, boolean inverse) {
        final float sign = (float) (inverse ? -1.0 : 1.0);
        ...
        for (int blockSize = 2; blockSize <= len; blockSize <<= 1) {
            final float angle = (float) (Math.PI * 2.0 / blockSize);
            final float cm1 = (float) (Math.cos(angle));

            //Use trig identity to quickly compute: sm1 = Math.sin(angle)
            //Block size >= 2  ergo   0 <= angle <= Math.PI  ergo  sin(angle) >= 0
            final float sm1 = (float) (sign * Math.sqrt(1.0 - cm1 * cm1));

            //Use trig identity to quickly compute: cm2 = Math.cos(2.0 * angle)
            final float cm2 = (float) (2.0 * cm1 * cm1 - 1.0);

            //Use trig identity to quickly compute: sm2 = Math.sin(2.0 * angle);
            final float sm2 = (float) (2.0 * sm1 * cm1);

            final float w = (float) (2.0 * cm1);
            ...
            for (int i = 0; i < len; i += blockSize) {
                ...
                for (int j = i * 2 + off, n = 0; n < blockEnd; j += 2, n++) {
                    ...
                    int k = j + blockEnd * 2;
                    float tr = ar0 * x[k] - ai0 * x[k + 1];
                    float ti = ar0 * x[k + 1] + ai0 * x[k];
    ...
    private static void reverseBitShuffle(float[] a, int offA, float[] b, int offB, int dim, int bits) {
